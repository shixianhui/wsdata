<?php
class Grasses_model extends CI_Model {

	private $_tableName = 'grasses';
	...
	public function gets_join_stores($select = '*', $strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'grasses.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select);
		$this->db->order_by($by, $order);
		$this->db->join('stores', "stores.id = grasses.store_id and stores.status = 1");
		$query = $this->db->get_where($this->_tableName, $strWhere, $limit, $offset);
		if ($query->num_rows() > 0) {
			$ret = $query->result_array();
		}
	...
	public function count_join_stores($strWhere = NULL)
    {
        $count = 0;
		$this->db->select("count(*) as 'count'");
		$this->db->join('stores', "stores.id = grasses.store_id and stores.status = 1");
