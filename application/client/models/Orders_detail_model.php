...
class Orders_detail_model extends CI_Model {
	...
	public function gets_join_orders($select = '*',$strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'orders_detail.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select);
		$this->db->order_by($by, $order);
	...
	public function count_join_orders($strWhere = NULL)
    {
        $count = 0;
		$this->db->select("count(*) as 'count'");
		$this->db->join('orders', "orders.id = orders_detail.order_id");
	...
	public function sum($column = 'reward',$strWhere = NULL) {
        $count = 0;
