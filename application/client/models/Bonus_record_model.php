<?php
class Bonus_record_model extends CI_Model {

	private $_tableName = 'bonus_record';
	...
	public function gets_join_user($select = '*',$strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'bonus_record.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select);
		$this->db->order_by($by, $order);
		$this->db->join('user', "user.id = bonus_record.user_id and user.display <> 3", 'left');
		$query = $this->db->get_where($this->_tableName, $strWhere, $limit, $offset);
		if ($query->num_rows() > 0) {
			$ret = $query->result_array();
		}
	...
	public function sum($column = 'amount',$strWhere = NULL) {
        $count = 0;
