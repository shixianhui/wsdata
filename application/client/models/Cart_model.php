...
class Cart_model extends CI_Model {
	...
	public function gets_join_combos($select = '*',$strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'cart.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select);
		$this->db->order_by($by, $order);
		$this->db->join('combos', "cart.item_id = combos.id and combos.display = 1");
		$query = $this->db->get_where($this->_tableName, $strWhere, $limit, $offset);
		if ($query->num_rows() > 0) {
			$ret = $query->result_array();
		}
	...
	public function gets_join_stores($select = '*',$strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'cart.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select);
		$this->db->order_by($by, $order);
		$this->db->group_by("cart.store_id");
		$this->db->join('combos', "cart.item_id = combos.id and combos.display = 1");
		$this->db->join('stores', "cart.store_id = stores.id and stores.status = 1");
		$query = $this->db->get_where($this->_tableName, $strWhere, $limit, $offset);
		if ($query->num_rows() > 0) {
			$ret = $query->result_array();
		}
	...
	public function get_join_combos($select = '*', $strWhere = NULL) {
		$ret = array();
		$this->db->select($select);
		$this->db->join('combos', "cart.item_id = combos.id and combos.display = 1");
		$query = $this->db->get_where($this->_tableName, $strWhere);
	...
	public function count_join_combos($strWhere = NULL)
    {
        $count = 0;
		$this->db->select("count(*) as 'count'");
		$this->db->join('combos', "cart.item_id = combos.id and combos.display = 1");
