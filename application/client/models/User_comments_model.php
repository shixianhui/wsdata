<?php
class User_comments_model extends CI_Model {

	private $_tableName = 'user_comments';
	...
	public function gets_join_user($select = '*',$strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'user_comments.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select);
		$this->db->order_by($by, $order);
		$this->db->join('user', "user.id = user_comments.user_id and user.display <> 3");
		$query = $this->db->get_where($this->_tableName, $strWhere, $limit, $offset);
		if ($query->num_rows() > 0) {
			$ret = $query->result_array();
		}
