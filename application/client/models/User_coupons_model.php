...
class User_coupons_model extends CI_Model {
	...
	private $_tableName = 'user_coupons';
	...
	public function gets_join_coupons($select = '*',$strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'user_coupons.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select, FALSE);
		$this->db->order_by($by, $order);
		$this->db->join('coupons', "coupons.id = user_coupons.coupon_id and coupons.status = 1");
		$query = $this->db->get_where($this->_tableName, $strWhere, $limit, $offset);
		if ($query->num_rows() > 0) {
			$ret = $query->result_array();
		}
	...
	public function get_join_coupons($select = '*', $strWhere = NULL) {
		$ret = array();
		$this->db->select($select);
		$this->db->join('coupons', "coupons.id = user_coupons.coupon_id and coupons.status = 1");
		$query = $this->db->get_where($this->_tableName, $strWhere);
	...
	public function count_join_coupons($strWhere = NULL)
    {
        $count = 0;
        $this->db->select("count(*) as 'count'");
		$this->db->join('coupons', "coupons.id = user_coupons.coupon_id and coupons.status = 1");
