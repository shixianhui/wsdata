...
class Combos_model extends CI_Model {
	...
	public function replace($data) {
        $ret = $this->db->replace($this->_tableName, $data);
        return $ret;
    }
	...
	public function gets_join_stores($select = '*', $strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'combos.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select, false);
		$this->db->order_by($by, $order);
		$this->db->join('stores', "stores.id = combos.store_id and stores.status = 1");
		$query = $this->db->get_where($this->_tableName, $strWhere, $limit, $offset);
		if ($query->num_rows() > 0) {
			$ret = $query->result_array();
		}
