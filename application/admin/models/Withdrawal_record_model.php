<?php
class Withdrawal_record_model extends CI_Model {

	private $_tableName = 'withdrawal_record';
	...
	public function gets_join_user($select = '*',$strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'withdrawal_record.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select);
		$this->db->order_by($by, $order);
		$this->db->join('user', "user.id = withdrawal_record.user_id");
		$query = $this->db->get_where($this->_tableName, $strWhere, $limit, $offset);
		if ($query->num_rows() > 0) {
			$ret = $query->result_array();
		}
	...
	public function gets_join_stores($select = '*',$strWhere = NULL, $limit = NULL, $offset = NULL, $by = 'withdrawal_record.id', $order = 'DESC') {
		$ret = array();
		$this->db->select($select);
		$this->db->order_by($by, $order);
		$this->db->join('stores', "stores.id = withdrawal_record.store_id");
		$query = $this->db->get_where($this->_tableName, $strWhere, $limit, $offset);
		if ($query->num_rows() > 0) {
			$ret = $query->result_array();
		}
	...
	public function count_join_user($strWhere = NULL)
    {
        $count = 0;
		$this->db->select("count(*) as 'count'");
		$this->db->join('user', "user.id = withdrawal_record.user_id");
