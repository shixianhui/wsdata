layui.define(['jquery','tree'], function (exports) {
    ...
    CustomSelect.prototype.render = function (options) {
        let el = options.el,
            ...
            checkBoxDiv = `<div id="${list}" class="layui-anim layui-anim-upbit" style="display: none;position: absolute; left: 0; padding: 5px 0; max-height: 350px; overflow-y: scroll; background: #FFFFFF; z-index:99; border:1px solid #eee;width:100%;"></div>`
        ...
        let handle = {
            ...
            init: function () {
                const _self = this
                $el.val(originVal)
                $el.parent().addClass('layui-form-select')
                $el.parent().append('<i class="layui-edge"></i>')
                ...
                tree.render({
                    ...
                    click: function (obj) {
                        if (type === 'radio') {
                            ...
                            $(obj.elem).find('.layui-tree-txt:first').css('color', 'red')
                            _self.onclick(obj)
                        }
                    },
                ...
                $(document).on('click', function(e){
                    if(e.target.id == el) {
                        $(listm).show()
                        $el.parent().addClass('layui-form-selected')
                    } else {
                        $(listm).hide()
                        $el.parent().removeClass('layui-form-selected')
                    }
                })
