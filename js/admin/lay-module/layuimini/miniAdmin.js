        ...
        listen: function () {
            ...
            $('body').on('click', '[data-clear]', function () {
                ...
                if (clearUrl != undefined && clearUrl != '' && clearUrl != null) {
                    let controller = $(".layui-tab-item.layui-show").find("iframe")[0].contentWindow.controller
                    clearUrl = clearUrl+'/'+controller;
                    $.getJSON(clearUrl, function (data, status) {
                        layer.close(loading);
                        if (data.code != 1) {
                            return miniAdmin.error(data.msg);
                        } else {
                            $(".layui-tab-item.layui-show").find("iframe")[0].contentWindow.location.href = $(".layui-tab-item.layui-show").find("iframe")[0].contentWindow.location.href;
